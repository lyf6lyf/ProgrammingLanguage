...
class LineSegment < GeometryValue
  ...
  def preprocess_prog
    if real_close(x1, x2)
      if real_close(y1, y2)
        Point.new(x1, y1)
      elsif y1 < y2 then self else LineSegment.new(x2,y2,x1,y1)
      end
    elsif x1 < x2 then self else LineSegment.new(x2,y2,x1,y1)
    end
  end
  def shift(dx,dy)
  ...
  def intersectWithSegmentAsLineResult seg
    if real_close(seg.x1, seg.x2)
      ...
      if real_close(segA.y2, segB.y1)
        ...
        LineSegment(segB.x1, segB.y1, segA.x2, segA.y2)
      end
    else
      if seg.x1 < x1
        segA, segB = seg, self
...
class Let < GeometryExpression
  ...
  def eval_prog env 
    r1 = @e1.eval_prog env
    @e2.eval_prog Array.new(env).unshift([@s, r1])
  end
  def preprocess_prog
